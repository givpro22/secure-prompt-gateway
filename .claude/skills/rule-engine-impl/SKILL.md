...
기획서 7장(정책·규칙·충돌 해결)·8.4(응답 형식)가 원본이다. 규칙 8종의 패턴은 `policy_rule` 테이블에서 읽는다. **Java 상수로 하드코딩하지 않는다** — 하면 정책을 DB에 둔 이유가 사라지고 "정책·규칙·임계값은 DB"라는 Config Isolation 주장이 무너진다.
...
1. **시드** — 규칙 8종이 다 들어갔나, 패턴이 온전한가, `department_policy` 2행이 있나
