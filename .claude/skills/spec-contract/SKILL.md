...
정책 편집 UI · 정책 시뮬레이터 · 로그인/인증 · 문서·이미지 첨부와 OCR · 임베딩/벡터 검색 · 사내 모델 호스팅 · 토큰 비용 집계

"금방 되는데요"가 이 프로젝트에서 가장 위험한 문장이다. 3일이고 4인이다.

### 상한이 움직인 자리 (2026-09-03, 기획서 0.6)

셋은 이제 **만든 것**이다. 다시 "범위 밖"이라고 안내하지 말 것.

| 항목 | 지금 |
|---|---|
| 출력(응답) 검사 | 구현. `phase=OUTPUT`으로 입력 파이프라인을 그대로 탄다 |
| 실제 LLM 호출 | 구현. `ANSWER_PROVIDER`가 mock·openai·claude를 고른다. **기본은 mock** |
| 표 파일 입력 | 구현. 프론트엔드가 텍스트로 뽑아 기존 `POST /messages`로 보낸다 (D17) |

셋 다 기존 파이프라인을 재사용하고 엔진을 고치지 않는다. **백엔드 첨부 업로드 API는 여전히 범위 밖이다** — 표 파일이 들어온 것은 파일이 아니라 텍스트이기 때문이다. 역할 검사(D24·D25)는 하지만 인증은 여전히 없다.
