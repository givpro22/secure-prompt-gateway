...
-- -----------------------------------------------------------------------------
-- 5. 홍보팀 감사 로그 5건 (message/inspection 104~108)
--
...
INSERT INTO message (message_id, user_id, original_text, submitted_text, status, created_at) VALUES
    (104, 5, '다음 주 보도자료 초안 톤을 좀 더 부드럽게 다듬어줘',
          '다음 주 보도자료 초안 톤을 좀 더 부드럽게 다듬어줘', 'ALLOWED', now() - interval '5 hours'),
    (105, 5, '기자 간담회 Q&A 예상 질문 10개만 뽑아줘',
          '기자 간담회 Q&A 예상 질문 10개만 뽑아줘', 'ALLOWED', now() - interval '4 hours'),
    (106, 5, '사내 공지문 문구 검토해줘 (담당자 연락처 [전화번호] 포함)',
          '사내 공지문 문구 검토해줘 (담당자 연락처 [전화번호] 포함)', 'MASKED', now() - interval '3 hours'),
    (107, 5, '행사 참석자 안내 메일 초안 써줘 (회신 주소 [이메일])',
          '행사 참석자 안내 메일 초안 써줘 (회신 주소 [이메일])', 'MASKED', now() - interval '2 hours'),
    (108, 5, '분기 홍보 성과 요약 슬라이드 목차 잡아줘',
          '분기 홍보 성과 요약 슬라이드 목차 잡아줘', 'ALLOWED', now() - interval '1 hours');
...
SELECT m.message_id, m.message_id, 'INPUT', ctx.policy_snapshot,
       jsonb_build_object(
           'matches',
           CASE WHEN m.status = 'MASKED' THEN jsonb_build_array(jsonb_build_object(
                    'code',           CASE m.message_id WHEN 106 THEN 'PII-PHONE-03' ELSE 'PII-EMAIL-04' END,
                    'category',       'PII',
                    'action',         'MASK',
                    'span',           jsonb_build_array(
                                          strpos(m.original_text,
                                                 CASE m.message_id WHEN 106 THEN '[전화번호]' ELSE '[이메일]' END) - 1,
                                          strpos(m.original_text,
                                                 CASE m.message_id WHEN 106 THEN '[전화번호]' ELSE '[이메일]' END) - 1
                                          + length(CASE m.message_id WHEN 106 THEN '[전화번호]' ELSE '[이메일]' END)),
                    'matchedKeyword', NULL::text,
                    'severity',       CASE m.message_id WHEN 106 THEN 'MEDIUM' ELSE 'LOW' END,
                    'obligation',     'LEGAL',
                    'source',         '개인정보보호법',
       ...
       'SKIPPED', NULL,
       CASE m.status WHEN 'MASKED' THEN 'MASK' ELSE 'ALLOW' END, 'RULE',
       m.created_at, m.created_at
  FROM message m CROSS JOIN seed_pr_ctx ctx
 WHERE m.message_id BETWEEN 104 AND 108;

INSERT INTO inspection_finding
    (inspection_id, source, rule_id, code, category, span_start, span_end, action, review_status)
SELECT m.message_id, 'RULE',
       CASE m.message_id WHEN 106 THEN 3 ELSE 4 END,
       CASE m.message_id WHEN 106 THEN 'PII-PHONE-03' ELSE 'PII-EMAIL-04' END,
       'PII',
       strpos(m.original_text, CASE m.message_id WHEN 106 THEN '[전화번호]' ELSE '[이메일]' END) - 1,
       strpos(m.original_text, CASE m.message_id WHEN 106 THEN '[전화번호]' ELSE '[이메일]' END) - 1
           + length(CASE m.message_id WHEN 106 THEN '[전화번호]' ELSE '[이메일]' END),
       'MASK', 'CONFIRMED'
  FROM message m
 WHERE m.message_id IN (106, 107);

DROP TABLE seed_pr_ctx;

-- -----------------------------------------------------------------------------
-- 6. 시퀀스 재동기화. V2와 같은 이유다 — PK를 명시 지정했으므로 맞춰두지 않으면
--    첫 API 호출이 duplicate key로 죽는다.
-- -----------------------------------------------------------------------------
