...
무엇을 보이는가 (파일 순서 = 발표 순서)
    01  통과        게이트웨이는 전부 막는 물건이 아니다
    02  마스킹      형식이 있는 것은 잡고, 명단에 없는 이름은 못 잡는다 (규칙의 한계)
    03a 차단        자격증명·주민번호는 나가기 전에 멈춘다
    03b 재전송      고쳐서 다시 보내면 통과한다
    04  부서 차이   같은 문장이 영업팀에서는 검토, 개발팀에서는 통과
    05  엠바고 차단 아직 때가 아니라서 막는다
    06  엠바고 해제 해제일이 지나면 같은 종류의 문장이 통과한다
    07  오탐        전화번호 형식의 부품 번호가 가려진다 → 해제 검토로 사람이 푼다
    08  출력 검사   나간 뒤 돌아온 답변이 코드를 되돌려주면 유출로 잡는다
