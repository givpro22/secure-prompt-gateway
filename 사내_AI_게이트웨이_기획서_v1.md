...
| ③ AI 응답(출력) 검사 | 설계만 | 입력 검사와 같은 파이프라인을 재사용하므로 ERD에 phase 컬럼만 남김 |
| ④ 프롬프트 효율·비용 관리 (F2) | 향후 확장 | 같은 로그를 재활용하는 두 번째 용도. 구현 시 범위가 두 배가 됨 |
| ⑤ 첨부파일 검사 | 설계만 | 문서 파싱·OCR이 3일 범위를 벗어남. 추출기를 어댑터로 분리해 두었음 |
...
| 테이블 | Core 8개 (DDL 실행) | Future 4개 (Logical Model에만 표기) |
| API | 7개 | 정책 생성·수정, 첨부 업로드 |
| 규칙 엔진 | REGEX 6종 + KEYWORD 2종, 부서별 매핑, 충돌 해결 | 체크섬 검증, 마스킹 전략 다양화 |
| AI | AiInspector 인터페이스 + Mock 구현체 + 202 비동기 | 실제 LLM 호출, 임베딩, 벡터 검색 |
| 검사 범위 | 입력 프롬프트 | 출력 응답, 첨부파일 |

### 0.4 AI 구현 여부

AI를 개발하지 않습니다. 가이드 2일차 항목에 "AI 서비스는 Mock API로 JSON 반환"이 명시되어 있고, 정량 루브릭 8개 기준에 AI 구현 항목이 없습니다. 요구되는 것은 다음 네 가지이며 모두 백엔드·프론트엔드 작업으로 충족됩니다.
...
| D11 | Case A의 실제 정규식 매칭 건수 | **원시 4건 → 억제 2건.** PII-EMAIL-04도 억제 대상 | 정규식을 실행해 확인함. `40ss@10.0.3.21`이 이메일로 오탐되나 SEC-DBURL-02 구간에 포함되어 억제됨. 단위 테스트 기대값을 이 과정으로 고정 | 10.4 |
...
```json
{
  ...
  "submittedText": "A사 차세대 프로젝트 오픈 일정이 언제였지?",
  "status": "PENDING_REVIEW",
  "policySnapshot": { "policies": [ { "code": "P-PII", "version": 3 },
                                    { "code": "P-SEC", "version": 7 },
...
| 단계 | 항목 | 변경 범위 |
|---|---|---|
| 1 | 맥락 판정 모델 사내 호스팅 (SGuard-v1, Safeguard by Kanana 등) | LlmAiInspector 구현, 환경변수만 |
| 2 | 근거 문서 검색 (RAG) — knowledge_source 테이블 검색으로 hits 대체 | 9.3의 3번 구성 요소 출처 변경 |
| 3 | 출력 검사 — phase=OUTPUT | 파이프라인 재사용, 화면 표시 추가 |
| 4 | 첨부파일 검사 — 추출기 어댑터 | attachment 테이블, 추출기만 추가. 엔진 무변경 |
